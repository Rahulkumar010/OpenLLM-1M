...
class WebUI:
    ...
    def __init__(self, agent: Union[Agent, MultiAgentHub, List[Agent]], chatbot_config: Optional[dict] = None):
        ...
        self.input_placeholder = chatbot_config.get('input.placeholder', '')
            ...
            with gr.Row(elem_classes='container'):
                with gr.Column(scale=4):
                    ...
                    input = mgr.MultimodalInput(placeholder=self.input_placeholder
                                                ,upload_button_props=dict(file_types=[".pdf", ".docx", ".pptx", ".txt", ".html", ".csv", ".tsv", ".xlsx", ".xls"] + ["." + file_type for file_type in common_programming_language_extensions]))
                ...
                with gr.Column(scale=1):
                    ...
                    if self.prompt_suggestions:
                        gr.Examples(
                            label='Recommend conversations',
                            examples=self.prompt_suggestions,
                            inputs=[input],
                        )
                    
                    streaming_agent = gr.Checkbox(
                        label='Streaming LLM',
                        interactive=True,
                        container=True,
                    )
